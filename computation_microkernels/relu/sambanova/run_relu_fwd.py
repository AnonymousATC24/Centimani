...
for dataType in ("bfloat16",): #np.float32, np.float16,
    ...
    for w, h, c, n, k, s, r, pad_w, pad_h, wstride, hstride in config:
        ...
        str_compile = "python relu_fwd.py compile -b " + str(n) + " -mb " + str(n) + " --dtype='" + dataType + "' --inference --w=" + str(w) + " --h=" + str(h) + " --c=" + str(c) + " --n=" + str(n) + " --output-folder='pef' --pef-name='relu_net'"
        print(str_compile)
        str_execute = "python relu_fwd.py run -b " + str(n) + " -mb " + str(n) + " --dtype='" + dataType + "' --inference --w=" + str(w) + " --h=" + str(h) + " --c=" + str(c) + " --n=" + str(n) + " --pef='pef/relu_net/relu_net.pef'"
        print(str_execute)
